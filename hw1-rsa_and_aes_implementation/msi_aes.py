### This is a template file for a simple AES function.
##
##  Please implement the provided functions and assure that your code
##  works correctly for the example given below
##
##  Name: <your name>
##  Date: <submission date>
##
import os
import sys
import math

# Rijndael S-box
sbox = []

iSbox =[]

Rcon = [ ]

mi2 = [

]

mi3 = [

]        

mi9 = [

]

mi11 = [

]

mi13 = [

]

mi14 = [

]



def rotWord(word): # rotate function that does logical shift since glorious python doesnt seem to support it natively
        rotword = word[1:len(word)]
        rotword.append(word[0])
        return rotword

def subWord(word):      #sbox substitution
        tsubword = bytearray(len(word))
        for i in range(len(word)):
                tsubword[i]=sbox[word[i]]
        return tsubword
                
def expandKey(key): # key expansion

        """Expands the key using the appropriate key scheduling """
#Nb = constant 4
#Nk = 4,6,8 keys size dependent
#Nr = 10, 12, 14

        # put your code here
        size = len(key)
        options = {16:10,24:12,32:14}
        Nr = options[size]      # select Nr depending on the key size
        Nb = 4
        Nk = size//4
        w = bytearray(Nb*(Nr+1)*4)
        i = 0
        for i in range(Nk):
                i4 = i*4
                #w[i4:i4+4] = key[i4:i4+4]
                w[i4] = key[i4]
                w[i4+1] = key[i4+1]
                w[i4+2] = key[i4+2]
                w[i4+3] = key[i4+3]
                
        #w[i] = key[4*i],key[4*i+1],key[4*i+2],key[4*i+3]
        #w[i] += key[4*i:4*i+4]

        i = Nk
        temp = bytearray(4)
        
        while i < Nb*(Nr+1):
                temp[0] = w[4*i-4]
                temp[1] = w[4*i-3]
                temp[2] = w[4*i-2]
                temp[3] = w[4*i-1]
                m = i//Nk
                if i%Nk == 0:   # do rotation and subs every Nk loop
                        rottemp = rotWord(temp)
                        subtemp = subWord(rottemp)
                        rtemp = Rcon[m]
                        subtemp[0] = subtemp[0]^rtemp
                        temp = subtemp
                        #print(temp)

                elif Nk > 6 and i % Nk == 4:
                        temp = subWord(temp)
                #w[i:i+4] = w[i-Nk:(i-Nk+4)] ^ temp
                w[i*4]=w[4*(i-Nk)] ^ temp[0]
                w[i*4+1]=w[4*(i-Nk)+1] ^ temp[1]
                w[i*4+2]=w[4*(i-Nk)+2] ^ temp[2]
                w[i*4+3]=w[4*(i-Nk)+3] ^ temp[3]
                i += 1
        
        return w

def enc_round(state,ekey):
        #print('encrypting one round')
        #print('initial state is ',state)

        #print('pre subbytes = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state = subbytes(state)
        #print('pre shiftRows = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state = shiftRows(state)
        #print('post shiftRows = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        
        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))
        #print('col0 - ',hex(ekey[0]),hex(ekey[1]),hex(ekey[2]),hex(ekey[3]))
        #print('col1 - ',hex(ekey[4]),hex(ekey[5]),hex(ekey[6]),hex(ekey[7]))
        #print('col2 - ',hex(ekey[8]),hex(ekey[9]),hex(ekey[10]),hex(ekey[11]))
        #print('col3 - ',hex(ekey[12]),hex(ekey[13]),hex(ekey[14]),hex(ekey[15]))

        #print('pre mixColumns = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state = mixColumns(state)
        #print('post mixColumns = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        
        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))

        #print('pre addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state = addroundkey(state,ekey)
        #print('post addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        
        #print('encrypted round')

        #print('encrypted state is ',state)
        return state

def enc_last(state,ekey):
        print('encrypting last round')
        state = subbytes(state)
        state = shiftRows(state)
        state = addroundkey(state,ekey)
        return state


def addroundkey(state,roundKey):
        for i in range(16):
                state[i] ^= roundKey[i]
        return state

def subbytes(state):
        """Performs SubBytes operation on the state."""
        # put your code here
        #print('subbyte oncesi state ')
        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))
        for i in range(16):
                hs = state[i]
                state[i] = sbox[hs]
        #print('subbyte sonrasi state ')
        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))  
        return state


def mult2(val):
        sign = 0
        if val>127:
                sign=1
        #print ('sign ',sign)
        const = 27 #h1b
        #print ('const ',hex(const))
        val2 = val << 1
        val3 = val2 & 255
        #print ('shifted val2 ',hex(val2))
        #print ('sign oncesi val ',hex(val3))
        if sign:
                val3=val3^const
                #print ('xored value ',hex(val3))
        #print('sign sonrasi val ',hex(val3))
        return val3
def mult3(val):
        org = val
        m2 = mult2(val)
        #print(hex(m2))
        #print(hex(org))
        val2 = m2 ^ org
        val3 = val2 & 255
        return val3

def mixColumns(state):
        """Performs mixColumns operation on the state."""

        # put your code here
        
        #mult0 = [2, 3, 1, 1]
        #mult1 = [1, 2, 3, 1]
        #mult2 = [1, 1, 2, 3]
        #mult3 = [3, 1, 1, 2]
        col0 = bytearray(4)
        col1 = bytearray(4)
        col2 = bytearray(4)
        col3 = bytearray(4)
        
        col0 = state[0:4]
        col1 = state[4:8]
        col2 = state[8:12]
        col3 = state[12:16]
        #x=mult2(col0[0])
        #print ('mult2 of D4 ',hex(x))

        #x = col0[2]
        #y = mult3(col0[2])

        #print ('x ',hex(x))
        #print ('mult of x ',hex(y))

        state[0] = mult2(col0[0]) ^ mult3(col0[1]) ^ col0[2] ^ col0[3]
        state[1] = col0[0] ^ mult2(col0[1]) ^ mult3(col0[2]) ^ col0[3]
        state[2] = col0[0] ^ col0[1] ^ mult2(col0[2]) ^ mult3(col0[3])
        state[3] = mult3(col0[0]) ^ col0[1] ^ col0[2] ^ mult2(col0[3])
        
        state[4] = mult2(col1[0]) ^ mult3(col1[1]) ^ col1[2] ^ col1[3]
        state[5] = col1[0] ^ mult2(col1[1]) ^ mult3(col1[2]) ^ col1[3]
        state[6] = col1[0] ^ col1[1] ^ mult2(col1[2]) ^ mult3(col1[3])
        state[7] = mult3(col1[0]) ^ col1[1] ^ col1[2] ^ mult2(col1[3])

        
        #print('qweqweqweqweqwe',hex(col2[0]))
        #x = mult2(col2[0])
        #y = mult3(col2[1])
        #print ('mult2 c20 ',hex(x))
        #print ('mult3 c21 ',hex(y))
        #print('col22',hex(col2[2]))
        #print('col23',hex(col2[3]))
        #print(' -*-*-*state 8 ',hex(state[8]))

        state[8] = mult2(col2[0]) ^ mult3(col2[1]) ^ col2[2] ^ col2[3]
        state[9] = col2[0] ^ mult2(col2[1]) ^ mult3(col2[2]) ^ col2[3]
        state[10] = col2[0] ^ col2[1] ^ mult2(col2[2]) ^ mult3(col2[3])
        state[11] = mult3(col2[0]) ^ col2[1] ^ col2[2] ^ mult2(col2[3])        

        state[12] = mult2(col3[0]) ^ mult3(col3[1]) ^ col3[2] ^ col3[3]
        state[13] = col3[0] ^ mult2(col3[1]) ^ mult3(col3[2]) ^ col3[3]
        state[14] = col3[0] ^ col3[1] ^ mult2(col3[2]) ^ mult3(col3[3])
        state[15] = mult3(col3[0]) ^ col3[1] ^ col3[2] ^ mult2(col3[3])
        
        return state

def iSubBytes(state):
        """Performs inverse SubBytes operation on the state."""

        # put your code here

        #print('pre iSubBytes = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        for i in range(16):
                hs = state[i]
                state[i] = iSbox[hs]
        #print('post iSubBytes = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        
        return state
      

def iShiftRows(state):
        """Performs inverse shiftRows operation on the state."""

        # put your code here
        newstate = bytearray(16)
        newstate[0] = state[0]
        newstate[4] = state[4]
        newstate[8] = state[8]
        newstate[12] = state[12]
        
        newstate[1] = state[13]
        newstate[5] = state[1]
        newstate[9] = state[5]
        newstate[13] = state[9]
        newstate[2] = state[10]
        newstate[6] = state[14]
        newstate[10] = state[2]
        newstate[14] = state[6]
        newstate[3] = state[7]
        newstate[7] = state[11]
        newstate[11] = state[15]
        newstate[15] = state[3]
        
        state = newstate
        return state

def shiftRows(state):
        """Performs shiftRows operation on the state."""

        # put your code here
        newstate = bytearray(16)
        newstate[0] = state[0]
        newstate[4] = state[4]
        newstate[8] = state[8]
        newstate[12] = state[12]
        newstate[1] = state[5]
        newstate[5] = state[9]
        newstate[9] = state[13]
        newstate[13] = state[1]
        newstate[2] = state[10]
        newstate[6] = state[14]
        newstate[10] = state[2]
        newstate[14] = state[6]
        newstate[3] = state[15]
        newstate[7] = state[3]
        newstate[11] = state[7]
        newstate[15] = state[11]
        
        state = newstate
        return state

def iMixColumns(state):
        """Performs inverse mixColumns operation on the state."""

        # put your code here

        #mult0 = [14,11,13,9]
        #mult1 = [9,14,11,13]
        #mult2 = [13,9,14,11]
        #mult3 = [11,13,9,14]

        col0 = bytearray(4)
        col1 = bytearray(4)
        col2 = bytearray(4)
        col3 = bytearray(4)
        
        col0 = state[0:4]
        col1 = state[4:8]
        col2 = state[8:12]
        col3 = state[12:16]
 
        state[0] = mi14[col0[0]] ^ mi11[col0[1]] ^ mi13[col0[2]] ^ mi9[col0[3]]
        state[1] = mi9[col0[0]] ^ mi14[col0[1]] ^ mi11[col0[2]] ^ mi13[col0[3]]
        state[2] = mi13[col0[0]] ^ mi9[col0[1]] ^ mi14[col0[2]] ^ mi11[col0[3]]
        state[3] = mi11[col0[0]] ^ mi13[col0[1]] ^ mi9[col0[2]] ^ mi14[col0[3]]
        
        state[4] = mi14[col1[0]] ^ mi11[col1[1]] ^ mi13[col1[2]] ^ mi9[col1[3]]
        state[5] = mi9[col1[0]] ^ mi14[col1[1]] ^ mi11[col1[2]] ^ mi13[col1[3]]
        state[6] = mi13[col1[0]] ^ mi9[col1[1]] ^ mi14[col1[2]] ^ mi11[col1[3]]
        state[7] = mi11[col1[0]] ^ mi13[col1[1]] ^ mi9[col1[2]] ^ mi14[col1[3]]

        state[8] = mi14[col2[0]] ^ mi11[col2[1]] ^ mi13[col2[2]] ^ mi9[col2[3]]
        state[9] = mi9[col2[0]] ^ mi14[col2[1]] ^ mi11[col2[2]] ^ mi13[col2[3]]
        state[10] = mi13[col2[0]] ^ mi9[col2[1]] ^ mi14[col2[2]] ^ mi11[col2[3]]
        state[11] = mi11[col2[0]] ^ mi13[col2[1]] ^ mi9[col2[2]] ^ mi14[col2[3]]

        state[12] = mi14[col3[0]] ^ mi11[col3[1]] ^ mi13[col3[2]] ^ mi9[col3[3]]
        state[13] = mi9[col3[0]] ^ mi14[col3[1]] ^ mi11[col3[2]] ^ mi13[col3[3]]
        state[14] = mi13[col3[0]] ^ mi9[col3[1]] ^ mi14[col3[2]] ^ mi11[col3[3]]
        state[15] = mi11[col3[0]] ^ mi13[col3[1]] ^ mi9[col3[2]] ^ mi14[col3[3]]
      
        return state

def dec_round(state,ekey):
        #print('decrypting one round')
        #print('pre iShiftRows = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('pre addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('roundkey = ',hex(ekey[0]),hex(ekey[1]),hex(ekey[2]),hex(ekey[3]))
        state=addroundkey(state,ekey)
        #print('post addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state=iMixColumns(state)
        #print('post iMixColumns = ',hex(ekey[0]),hex(ekey[1]),hex(ekey[2]),hex(ekey[3]))
        state=iShiftRows(state)        
        #print('post iShiftRows = ',hex(ekey[0]),hex(ekey[1]),hex(ekey[2]),hex(ekey[3]))
        state=iSubBytes(state)
        #print('post iSubBytes = ',hex(ekey[0]),hex(ekey[1]),hex(ekey[2]),hex(ekey[3]))
        return state

def dec_last(state,ekey):
        #print('decrypting first round')
        #print('pre addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #print('pre addroundkeeey = ',hex(ekey[0]),hex(ekey[1]),hex(ekey[2]),hex(ekey[3]))
        state = addroundkey(state,ekey)
        #print('post addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state = iShiftRows(state)        
        #print('post iShiftRows = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]),hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
        state = iSubBytes(state)
        #print('post iSubBytes = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        
        return state

def AES_encrypt(plaintext,key):
        """Performs an encryption on the plaintext """
        # init state
        state = bytearray(pt)

        # put your code here
        ekey = expandKey(key)

       # for i in range(len(ekey)//4):
       #         print(hex(ekey[4*i]),hex(ekey[4*i+1]),hex(ekey[4*i+2]),hex(ekey[4*i+3]))
        
        addroundkey(state,ekey[0:16])
        
        # check the key size
        if len(key)==16:
                #do 9 rounds
                print('key size is 128-bits')
                for i in range(9):
                       # print('plain state ')
                       # print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
                       # print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
                        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))        

                        print('encrypting round ',i+1)
                        state = enc_round(state,ekey[(i+1)*16:(i+2)*16])
                       # print('encrypted state ')
                        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
                        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
                        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))  
                        
                                                
                i = 10
                state = enc_last(state,ekey[(i)*16:(i+1)*16])
                
        elif len(key)==24:
                #do 12 rounds
                print('key size is 196-bits')
                for i in range(11):
                        #print('plain state ')
                        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
                        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
                        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))        

                        print('encrypting round ',i+1)
                        state = enc_round(state,ekey[(i+1)*16:(i+2)*16])
                        #print('encrypted state ')
                        #print('col0 - ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                        #print('col1 - ',hex(state[4]),hex(state[5]),hex(state[6]),hex(state[7]))
                        #print('col2 - ',hex(state[8]),hex(state[9]),hex(state[10]),hex(state[11]))
                        #print('col3 - ',hex(state[12]),hex(state[13]),hex(state[14]),hex(state[15]))
                i = 12
                state = enc_last(state,ekey[(i)*16:(i+1)*16])
                        
        elif len(key)==32:
                #do 14 rounds
                print('key size is 256-bits')
                for i in range(13):
                        print('encrypting round ',i+1)
                        state = enc_round(state,ekey[(i+1)*16:(i+2)*16])
                i = 14
                state = enc_last(state,ekey[(i)*16:(i+1)*16])
                
        else:
                print("incorrect key size, what's the matter with ya!")
                
        # return ciphertext
        return state

def AES_decrypt(ct,key):
        """Performs an decryption on the ciphertext """

        # init state
        state = bytearray(ct)
        
        # put your code here
        ekey = expandKey(key)
        ln = len(ekey)
        #for i in range(ln//4):
        #        print('key ',i,'  ',hex(ekey[4*i]),hex(ekey[4*i+1]),hex(ekey[4*i+2]),hex(ekey[4*i+3]))
       
        #print('pre dec_last = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        state = dec_last(state,ekey[ln-16:ln])
        #print('pre dec_last = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))

        #print('pre addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
        #state = addroundkey(state,ekey[ln-16:ln])        
        #print('post addroundkey = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))

        # check the key size
        if len(key)==16:
                #do 9 regular rounds
                print('key size is 128-bits')
                #print(ln)
                
                for i in range(9):
                        print('decrypting round ',i+1)
                        #print('pre dec_round = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                        state = dec_round(state,ekey[(ln-(16*(i+2))):(ln-(16*(i+1)))])
                        #print('post dec_round = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                i = 9
                state=addroundkey(state,ekey[(ln-(16*(i+2))):(ln-(16*(i+1)))])
                
        elif len(key)==24:
                #do 11 regular rounds
                print('key size is 196-bits')
                
                for i in range(11):
                        print('decrypting round ',i+1)
                       # print('pre dec_round = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                        state = dec_round(state,ekey[(ln-(16*(i+2))):(ln-(16*(i+1)))])
                       # print('post dec_round = ',hex(state[0]),hex(state[1]),hex(state[2]),hex(state[3]))
                i = 11
                #state = dec_last(state,ekey[(i)*16:(i+1)*16])
                state=addroundkey(state,ekey[(ln-(16*(i+2))):(ln-(16*(i+1)))])
                
        elif len(key)==32:
                #do 13 regular rounds
                print('key size is 256-bits')
                
                for i in range(13):
                        print('decrypting round ',i+1)
                        state = dec_round(state,ekey[(ln-(16*(i+2))):(ln-(16*(i+1)))])
                i = 13
                #state = dec_last(state,ekey[(i)*16:(i+1)*16])
                state=addroundkey(state,ekey[(ln-(16*(i+2))):(ln-(16*(i+1)))])
        else:
                print("incorrect key size, what's the matter with ya!")        
        
        # return ciphertext
        return state


### Testing your code:

# initializing sample inputs (see FIPS 197):
#pt  = bytearray.fromhex('32 43 f6 a8 88 5a 30 8d 31 31 98 a2 e0 37 07 34')
#key = bytearray.fromhex('2b 7e 15 16 28 ae d2 a6 ab f7 15 88 09 cf 4f 3c')
#ct  = bytearray.fromhex('39 25 84 1d 02 dc 09 fb dc 11 85 97 19 6a 0b 32')

#192 bit key vectors
#pt = bytearray.fromhex('00112233445566778899aabbccddeeff')
#key = bytearray.fromhex('000102030405060708090a0b0c0d0e0f1011121314151617')
#ct = bytearray.fromhex('dda97ca4864cdfe06eaf70a0ec0d7191')

#256 bit key vectors
pt = bytearray.fromhex('00112233445566778899aabbccddeeff')
key = bytearray.fromhex('000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f')
ct = bytearray.fromhex('8ea2b7ca516745bfeafc49904b496089')

#key = bytearray.fromhex('00 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15')
#ct  = bytearray.fromhex('E5 5C D4 A8 EE E5 7D 26 1C 16 CA FE C9 40 A9 44')


#fips test vectors
#pt  = bytearray.fromhex('00112233445566778899aabbccddeeff')
#key = bytearray.fromhex('000102030405060708090a0b0c0d0e0f')
#ct = bytearray.fromhex('69c4e0d86a7b0430d8cdb78070b4c55a')


size = len(key)
options = {16:10,24:12,32:14}
Nr = options[size]
Nb = 4
Nk = size//4
w = bytearray(Nb*(Nr+1))
i = 0
for i in range(Nk):
        i4 = i*4
        w = key[i4:i4+4]

        
# print(hex(int.from_bytes(pt,'big')))

my_ct = AES_encrypt(pt,key)
if ct==my_ct:
        print('Good job!')
else:
        print('AES_encrypt Still some more error fixing needed')


my_pt = AES_decrypt(ct,key)
if pt==my_pt:
        print('Good job!')
else:
        print('AES_decrypt Still some more error fixing needed')
                

